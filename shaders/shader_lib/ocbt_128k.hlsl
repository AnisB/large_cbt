...
#include "shader_lib/OCBT_generic.hlsl"
...
void reduce_first_pass(uint dispatchThreadID, uint groupIndex)
{
    ...
    if (groupIndex % 2 == 0)
        gs_cbtTree[level0Offset + dispatchThreadID / 2] = _CBTBufferRW[level0Offset + dispatchThreadID / 2];
    ...
    if (groupIndex % 2 == 0)
        _CBTBufferRW[level2Offset + dispatchThreadID / 2] = gs_cbtTree[level2Offset + dispatchThreadID / 2];

    const uint level3Offset = OCBT_depth_offset[TREE_LAST_LEVEL - 2] / 32;
    if (groupIndex % 4 == 0)
        _CBTBufferRW[level3Offset + dispatchThreadID / 4] = gs_cbtTree[level3Offset + dispatchThreadID / 4];
}
